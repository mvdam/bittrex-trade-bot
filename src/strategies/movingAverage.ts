...
export const movingAverageStrategy: IStrategyConstructor = (
  config: IMovingAverageStrategy
): IStrategy => (marketState: IMarketState): IStrategyDecision => {
  ...
  const shouldSell = (): boolean =>
    movingAvg !== 0 && currentPrice >= sellTarget(movingAvg, config.minProfit)

  const shouldBuy = (): boolean =>
    movingAvg !== 0 && currentPrice <= buyTarget(movingAvg, config.minProfit)
