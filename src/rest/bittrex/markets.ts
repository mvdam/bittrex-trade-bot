...
import {
  IBittrexMarket,
  IBittrexMarketTicker,
  IBittrexMarketHistory
} from '../../interfaces/bittrex'
...
export const fetchBittrexMarketTicker = (
  market: IBittrexMarket
): Observable<IBittrexMarketTicker> =>
  fetchBittrexObservable(
    `https://bittrex.com/api/v1.1/public/getticker?market=${market.BaseCurrency}-${
...
export const fetchMarketHistory = (market: IBittrexMarket): Observable<IBittrexMarketHistory[]> =>
  fetchBittrexObservable(
    `https://bittrex.com/api/v1.1/public/getmarkethistory?market=${market.BaseCurrency}-${
      market.MarketCurrency
    }`
  )
...
export const fetchBittrexObservable = (api: string): Observable<any> =>
  fetchObservable({ api }).map(json => json.result)
...
export const signedFetchBittrexObservable = (api: string): Observable<any> => {
  ...
  return fetchObservable(fetchConfig).map(json => json.result)
}
